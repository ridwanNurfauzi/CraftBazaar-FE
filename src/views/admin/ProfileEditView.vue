...
export default {
    ...
    methods: {
        ...mapActions(useAdminAuthStore, ['getProfile', 'updateProfile']),
        changePhoto() {
        ...
        async performUpdateProfile() {
            ...
            try {
                ...
                const response = await this.updateProfile(fData, this.adminData.id);
    ...
    async mounted() {
        await this.getProfile().then(async () => {
            ...
            if (!!this.adminData?.photo)
                await fetch(`${this.API_URL}/public/images/profiles/admin/${this.adminData.photo}`)
                    .then(x => x.blob())
                    .then(y => {
                        this.photoFile = new File([y], `${this.adminData.photo}`);
