<template>
    <div class="flex-1 flex flex-col">
        <div class="w-full xl:max-w-7xl xl:mx-auto">
            <div class="m-4">
                <h1 class="text-center font-medium text-lg">Ubah Profil</h1>
            </div>
            <div class="md:grid md:grid-cols-3 md:gap-3 mx-4 md:mx-10">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="description" id="description" rows="7" v-model="description"
...
export default {
    ...
    methods: {
        ...mapActions(useSellerAuthStore, ['getProfile', 'updateProfile']),
        changePhoto() {
        ...
        async performUpdateProfile() {
            ...
            try {
                ...
                const response = await this.updateProfile(fData, this.sellerData.id);
    ...
    async mounted() {
        await this.getProfile().then(async () => {
            this.email = this.sellerData?.email ?? '';
            this.name = this.sellerData?.name ?? '';
            this.description = this.sellerData?.description ?? '';
            if (!!this.sellerData?.photo)
                await fetch(`${this.API_URL}/public/images/profiles/seller/${this.sellerData.photo}`)
                    .then(x => x.blob())
                    .then(y => {
                        this.photoFile = new File([y], `${this.sellerData.photo}`);
